...
public class StudentSelectOneMenu {
	...
	private static void selectOneStudent(Connection conn) {
		...
		try(
			...
			System.out.println("조회 결과 첫번째 컬럼의 값 : " + rs.getString(2));
			System.out.println("조회 결과 첫번째 컬럼의 값 : " + rs.getInt(3));
			System.out.println("조회 결과 첫번째 컬럼의 값 : " + rs.getString(4));
			
			System.out.println("다음 조회 결과 행이 또 있을까요 ?" + rs.next());
		}
		catch (SQLException e) {
			System.out.println("데이터 조회에 문제가 생겼습니다. 상세내용 - " + e.getMessage());
			//결과 집합을 모두 소모했음 -> 조회 결과가 없는데 rs.getXXXX 메소드 실행할 때의 예외 메세지.
		}
		sc.close();
