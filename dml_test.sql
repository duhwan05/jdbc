...
insert into TMP_STUDENT values ('2023001','김땡땡',16,'경기도');
insert into TMP_STUDENT values ('2023061','박두환',26,'서울');
insert into TMP_STUDENT values ('2023091','김경민',24,'서울');
...
-- delete 삭제 테스트
delete from TMP_STUDENT where stuno = '2023001';

select * from TBL_STUDENT where stuno = '2023002';

-- 여기서부터는 다른 테이블로 연습 합니다.
/*
1. 회원 로그인 - 간단한 회원 아이디를 입력해서 존재하면 로그인 성공
2. 상품 목록 보기
3. 상품명으로 검색하기
...
-- tbl_buy 테이블에는 외래키도 2개 있습니다. (j_buy 외래키 설정 제외하고 하겠습니다.)
...
insert into J_Product values ('APP99','B1','씻은사과 10개',25000);

-- J_buy 테이블에 사용할 시퀀스
drop sequence jbuy_seq;		-- 시퀀스 삭제
create sequence jbuy_seq start with 1007;		-- 적절한 시작값으로 다시 생성하기
select jbuy_seq.nextval from dual;

create sequence MEMER_TBL_02 memberseq start with 100007;
...
delete from j_buy where buy_seq = '1010';
...
-- 자주 사용될 join 결과는 view 로 만들기. view 는 create or replace 로 생성후에 수정까지 가능.
